...
import { ChevronLeft, Share2, TrendingUp, HelpCircle, ArrowRight, ShieldCheck } from 'lucide-react';
...
export default function HargaDetail() {
    ...
    return (
      <div className="bg-surface-container-lowest border border-outline-variant p-4 rounded-lg shadow-sm">
        <div className="flex justify-between items-center mb-3">
          <span className="text-label-md font-bold font-jakarta text-on-surface uppercase tracking-wider">Tren Bulanan (30 Hari Terakhir)</span>
          <span className="text-body-sm font-bold text-secondary bg-secondary-fixed px-2 py-0.5 rounded">Rerata: {formatRupiah(Math.round(prices.reduce((a,b)=>a+b,0)/prices.length))}</span>
        </div>

        <div className="relative">
          <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible font-jakarta">
            {/* Horizontal Grid lines */}
            <line x1={padding} y1={padding} x2={width - padding} y2={padding} stroke="var(--color-surface-container-high)" strokeDasharray="3" />
            <line x1={padding} y1={height / 2} x2={width - padding} y2={height / 2} stroke="var(--color-surface-container-high)" strokeDasharray="3" />
            <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} stroke="var(--color-surface-container-high)" strokeDasharray="3" />
            ...
            <text x={padding - 15} y={padding + 4} className="text-[8px] font-bold fill-on-surface-variant font-jakarta">{Math.round(max/1000)}k</text>
            <text x={padding - 15} y={height - padding + 2} className="text-[8px] font-bold fill-on-surface-variant font-jakarta">{Math.round(min/1000)}k</text>
  ...
  return (
    <div className="flex-1 pb-24 bg-surface text-on-surface">
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-lg overflow-hidden flex flex-col p-5 shadow-sm">
          <div className="flex gap-4">
            ...
            <div className="flex-1 flex flex-col justify-between py-1">
              <div>
                ...
                <h2 className="font-fraunces text-headline-md font-bold text-primary leading-tight mt-1.5">{item.name}</h2>
              </div>
              ...
              <div className="flex items-baseline gap-1 mt-1">
                ...
                <span className="text-body-sm text-on-surface-variant font-jakarta font-medium">{item.unit}</span>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-outline-variant/40 mt-4 pt-4 grid grid-cols-3 gap-2 text-center text-body-sm">
            <div className="border-r border-outline-variant/30">
              <span className="text-body-sm uppercase font-bold text-on-surface-variant font-jakarta">Delta Harian</span>
              <span className={`block font-bold mt-0.5 ${item.isUp ? 'text-on-tertiary-container' : 'text-secondary'}`}>
                {item.isUp ? '▲ +' : '▼ -'}{item.deltaPercent}%
              </span>
            </div>
            <div className="border-r border-outline-variant/30">
              <span className="text-body-sm uppercase font-bold text-on-surface-variant font-jakarta">Rerata Regional</span>
              <span className="block font-bold mt-0.5 text-primary">Jawa Barat</span>
            </div>
            <div>
              <span className="text-body-sm uppercase font-bold text-on-surface-variant font-jakarta">Validitas</span>
      ...
      {/* Monthly Line Graph */}
      <div className="px-5 mt-6">
        {renderLineChart(item.history30Days)}
      </div>

      {/* Market Benchmarks section */}
      <div className="px-5 mt-6">
        <h3 className="font-fraunces text-body-lg font-bold text-primary mb-3">Banding Harga Pasar Induk</h3>
        <div className="bg-surface-container-lowest border border-outline-variant rounded-lg overflow-hidden shadow-sm">
          {marketComparisons.map((comp, idx) => (
            ...
            >
              <div>
                <span className="font-jakarta font-bold text-body-sm text-on-surface block">{comp.market}</span>
      ...
      <div className="px-5 mt-8">
        <div className="bg-primary/5 border border-primary/20 p-5 rounded-lg flex flex-col gap-4 text-center">
          <p className="font-jakarta text-body-sm text-on-surface leading-relaxed">
            Butuh komoditas <b className="font-bold">{item.name}</b> dengan harga bersaing langsung dari petani Garut?
          </p>
          <Button
